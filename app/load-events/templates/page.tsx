  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Load Events — Templates</h1>
          <p className="mt-1 text-sm text-muted-foreground">Saved reference captures used to diff future runs against.</p>
        </div>
        <Button render={<Link href="/load-events/templates/new" />} nativeButton={false}>
        ...
        <Card className="p-0">
          <ul className="divide-y divide-border">
            {allTemplates.map((t) => (
              <li key={t.id}>
                <Link
                  href={`/load-events/templates/${t.id}`}
