...
/** Path extensions that are a link target but never a page beacon should check. */
...
/**
 * Discovers the set of pages a content check should visit. Tries sitemap.xml first —
 * cheap, and already the site's own declaration of what it considers a real page — then
 * falls back to a same-origin BFS crawl from the root when no sitemap exists (or it's
 * empty), since a site with no sitemap is not a site with no pages.
 */
